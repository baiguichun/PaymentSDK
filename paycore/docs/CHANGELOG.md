...
- 统一使用配置的 `networkTimeout` 和 `initialQueryDelayMs`，不再硬编码 10s/200ms。
- 网络请求参数做 URL 编码，避免特殊字符导致请求非法。
- 调试状态输出获取查询任务列表时使用快照，避免遍历期间并发修改。
- 网络层迁移到 Retrofit + OkHttp，使用统一的超时和字符串解析。
